...
export const getJoinedRoomMessanges = async (server, db) => {
    server.get("/data/message/user-messanges", verifyJWT, async (req: express.Request, res: express.Response) => {
        ...
        try {
            // if (!msgIdExist) {
            const room = await db.prepare("SELECT msg.id, msg.sender_id, msg.receiver_id, msg.room, msg.message, msg.timestamp, msg.deleted, msg.reported, msg.edited_message, msg.socket_id FROM message msg INNER JOIN joined_room jr on jr.room_id = msg.receiver_id WHERE jr.room_id = ?;").all(user.id);
            return res.json(room).status(200)
            // } else {
            //     return res.status(400).send({message: "Room id not found!"})
            // }
